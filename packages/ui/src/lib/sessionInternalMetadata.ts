...
export const isOpenChamberInternalSessionEvent = (
  ...
  const properties = (event as {
    properties?: { info?: Session & { sessionID?: string }; part?: { sessionID?: string }; sessionID?: string }
  }).properties
  const info = properties?.info
  const sessionID = properties?.sessionID ?? properties?.part?.sessionID ?? info?.sessionID ?? info?.id
