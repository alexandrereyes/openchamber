...
describe('parseConnectionPayload', () => {
  ...
  test('recognizes relay offers with embedded token and grant', () => {
    ...
    if (!payload || 'pairing' in payload) throw new Error('expected a relay connection payload');
  ...
  test('malformed relay offers fall through to direct parsing rules', () => {
    // mode=relay but no fragment payload → not a valid offer, and no legacy
    // (server+token) params either → rejected entirely, exactly like before
    // relay support.
    expect(parseConnectionPayload('openchamber://connect?v=1&mode=relay')).toBeNull();
    // Legacy v1 link that also carries an unrelated mode param keeps direct
    // parsing (server+token are both required for a legacy link).
    const direct = parseConnectionPayload('openchamber://connect?v=1&mode=relay&server=http%3A%2F%2Fhost.example&token=tok');
    expect(direct).toEqual({ url: 'http://host.example', clientToken: 'tok', label: undefined });
  });
});
