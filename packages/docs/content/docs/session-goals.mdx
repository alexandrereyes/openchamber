...
- The strip above the composer shows the goal's latest progress note, status, and token usage, with an inline pause/resume button.
- The target button stays lit while the goal runs (blue), turns green on completion, and red when blocked or out of budget. Press it to open the goal dialog: edit the objective or budget, or remove the goal (a completed goal is removed before starting a new one).
