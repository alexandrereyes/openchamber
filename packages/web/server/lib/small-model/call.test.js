...
describe('callSmallModel — structured output', () => {
  ...
  it.each([
    ['anthropic', { anthropic: { type: 'api', key: 'sk-ant' } }, 'claude-haiku-4-5'],
    ['google', { google: { type: 'api', key: 'google-key' } }, 'gemini-2.5-flash'],
  ])('classifies an explicit %s schema rejection', async (providerID, auth, modelID) => {
    fetchMock.mockResolvedValue(new Response(JSON.stringify({
      error: { message: 'structured output schema is not supported' },
    }), { status: 400 }));
