...
import { __testing, classifyOpenChamberInternalSessionEvent, internalSessionMetadata, isOpenChamberInternalSessionEvent, trackOpenChamberInternalSession } from './internal-sessions.js';
...
describe('internal session event registry', () => {
  ...
  it('recognizes nested message and part events for a tracked session', () => {
    trackOpenChamberInternalSession('ses_nested');
    expect(isOpenChamberInternalSessionEvent({
      type: 'message.updated', properties: { info: { id: 'msg_1', sessionID: 'ses_nested' } },
    })).toBe(true);
    expect(isOpenChamberInternalSessionEvent({
      type: 'message.part.updated', properties: { part: { id: 'part_1', sessionID: 'ses_nested' } },
    })).toBe(true);
  });
